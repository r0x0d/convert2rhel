...
def test_verify_initramfs_file(latest_installed_kernel, subprocess_output, expected, tmpdir, caplog, monkeypatch):
    ...
    monkeypatch.setattr(checks, "run_piped_subprocess", mock.Mock(return_value=subprocess_output))
    result = checks._verify_initramfs_file(latest_installed_kernel)
    assert result == expected

    if not expected:
        assert "stderr output: %s" % subprocess_output[1] in caplog.records[-1].message
...
def test_check_kernel_boot_files(pretend_os, tmpdir, caplog, monkeypatch):
    ...
    monkeypatch.setattr(checks, "VMLINUZ_FILEPATH", vmlinuz_file)
    monkeypatch.setattr(checks, "INITRAMFS_FILEPATH", initramfs_file)
    monkeypatch.setattr(checks, "run_subprocess", mock.Mock(return_value=rpm_last_kernel_output))
    monkeypatch.setattr(checks, "run_piped_subprocess", mock.Mock(return_value=("test", "", 0)))
...
def test_check_kernel_boot_files_missing(
    ...
    if create_initramfs:
        ...
        monkeypatch.setattr(checks, "INITRAMFS_FILEPATH", initramfs_file)
        # If we have a file, let's mock the output of this function depending on
        # certain cases.
        monkeypatch.setattr(checks, "run_piped_subprocess", mock.Mock(return_value=run_piped_subprocess))
    else:
