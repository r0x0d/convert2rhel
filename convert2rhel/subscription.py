...
def unregister_system():
    ...
    # We are calling run_subprocess with rpm here because of a bug in OL/CentOS 7
    # in which the process always exit with 1 in case of a rollback when
    # KeyboardInterrupt is raised, so, to avoid many changes and different
    # conditionals, we are doing a simple call for rpm to verify if
    # subscription-manager is installed on the system.
    # This is the current line on `rpm` that causes the process to exit with
    # any interaction with the yum library
